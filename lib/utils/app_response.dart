...
import 'package:webapp/model_responce.dart';

class AppResponse extends Response {
  AppResponse.badRequest({String? title, String? details})
      : super.badRequest(body: ErrorResponse(title: title ?? "Bad request", details: details));

  AppResponse.unauthorized({String? title, String? details})
      : super.unauthorized(body: ErrorResponse(title: title ?? "Unauthorized", details: details));

  AppResponse.forbidden({String? title, String? details})
      : super.forbidden(body: ErrorResponse(title: title ?? "Forbidden", details: details));
  
  AppResponse.notFound({String? title, String? details})
      : super.notFound(body: ErrorResponse(title: title ?? "Not found", details: details));
  ...
  static ErrorResponse _getResponseModel(error, String? message) {
    if (error is QueryException || error is JwtException) {
      return ErrorResponse(
          details: error.toString(), title: message ?? error.message.toString());
    }
